...
| Strict source badges | **You** means you explicitly asked for that ledger entry. A normal work request captured automatically is **Agent**; hovering the badge recalls the specific discussion point that caused its capture. |
...
| A plain-words detail disclosure | Hover or focus the small caret above the drag grip—or tap it—and a self-contained note restores the feature, problem, stakes, and exact decision without assuming you remember the task. |
| Open one item in Codex | Use the compact ↗ action to open a fresh Codex task with that item's saved context and an unsent request for a simple explanation plus one next action. |
...
In Codex, the recommended item can end with a hover-only ↗: its deliberately non-existent destination contains the item’s full explanation. Hover to read it; clicking does not open a file. Use this only when the user has confirmed that their host shows destination text. The separate Full outstanding items link still opens the editable ledger.

## Full outstanding items is an editor, not a raw file

Because the footer names one item, **Full outstanding items** is where the rest of it lives — a private local HTML view instead of a huge Markdown or JSON file. Active work sits under **Outstanding**. At rest, the compact composite reference (`OI-12-P1`) sits in a thin metadata line above the task text, reclaiming the width a separate ID column would waste. The real checkbox stays beside the content. The P0–P3 suffix is an accessible inline control: P0 is highest, P2 is the neutral default, and P3 is lowest. Changing it updates the same item rather than renaming its permanent `OI-n` key. A tiny **You** or **Agent** pill flows immediately after the text when the origin is known, so it does not reserve a separate column or force early wrapping. **You** is deliberately strict: it appears only when you explicitly asked to add that specific thing to Outstanding Items. If you merely requested or discussed the underlying work and the agent captured it automatically, the pill is **Agent**. Hover the pill to recall the exact discussion point that caused the capture — for example, “An agent added this because the deployment still needed your approval.” Older items keep honest missing-trigger data rather than receiving invented history. Click the text to create an inline editor; no blank input exists before that interaction. Automatic items stay sensibly ordered by actionable status, then P0→P3 priority, then newest relevance, so a high-priority peer does not remain buried. Drag with the reorder grip or use the keyboard move controls to set a lasting manual position; automatic reconciliation leaves that chosen slot alone. Checking a task complete moves it to the bottom and shows a temporary snackbar with **Undo**.

The whole row no longer triggers item details on hover. A small caret sits above the existing drag grip, using that same action column instead of taking width from the task text. Hover or keyboard-focus the caret to preview the item's ID, friendly state phrase, and short action-first explanation; click or tap it where hover is unavailable. Double-click the row to keep the same note open, or focus the task text and press `Alt+Enter`. The pinned explanation behaves like a non-modal dialog: close it from the top-right button or with `Escape`, and double-click its text normally to select it without activating the item underneath. After a pointer-opened dialog, `Escape` also clears the double-click selection instead of leaving the task looking focused for editing; keyboard-opened dialogs retain keyboard focus. A single task-text click still edits. Explanations use imperative Git commit-subject style — `Write a LinkedIn post…`, not `This is the idea to…` — and assume you may return months later with no task context. Each note names the feature or surface, the concrete problem or outcome, useful stakes, and the exact decision or action still needed; it explains domain jargon instead of relying on old item numbers, task titles, or remembered architecture. Items saved before that field existed start their fallback with the item title, and every row's text is rendered as text, never as markup.

The hover preview and expanded note show when an item was added, with its full date, local time, seconds, and timezone. New captures save that time once; editing, reordering, completing, or reopening an item preserves it. Older items whose creation time was not recorded say so instead of showing a guessed date.

An active row also keeps a compact ↗ **Open in Codex** action visibly available in the existing controls column, so it adds no width to the task text and does not depend on discovering a hover-only control. The quieter drag and keyboard-move controls still reveal on hover or focus. The action opens a fresh normal Codex task with a prefilled prompt containing only that item's saved context, asks for a simple explanation and the single next action, and explicitly tells Codex to stop there. The prompt is not sent automatically, the ledger is not changed, and completed or transferred rows do not show the action.

There is still only one ledger: the task-owned `outstanding-items.json`. The UI reads and atomically writes that file through a token-protected server bound to `127.0.0.1`; it stores no copy in the HTML or browser storage. Its private connection record keeps the same local URL through normal restarts, so an old tab reconnects instead of dying with `Failed to fetch`. The server snapshots and fingerprints its generic HTML, CSS, and JavaScript and fingerprints its local Python runtime at launch; a later `start` replaces older UI or server behavior instead of silently reusing it after a plugin refresh. Agent-side changes update the same JSON, and an open page notices a new revision within two seconds. If two edits race, the stale one is rejected and reloaded instead of overwriting newer work. Whenever an agent touches that ledger, it also checks the recorded evidence and moves genuinely verified or deliberately dropped items to Done. It does not leave completed **Agent** items open merely to ask you to accept proof it already has.
...
store the exact discussion point that caused each known-origin capture,
