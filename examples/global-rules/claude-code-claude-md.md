...
- In a Git-project session, create or resolve this chat's ledger under
...
Make two unrelated requests in one session and look at the end of the final response. If it does not start directly with one `**OI-n-Px …** — status` recommendation — or if it comes back as a labelled heading, multi-section list, count, or Done pile — or if the same block also shows up in the progress messages before it:
