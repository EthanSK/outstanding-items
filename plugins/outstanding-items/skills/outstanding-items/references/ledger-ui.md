...
Use the equivalent `~/.claude/skills/` path under Claude Code. The command prints `LEDGER_URL`, `LEDGER_PID`, and `LEDGER_LOG`. It reuses the exact healthy process for that ledger only after its API and browser assets respond and both the UI-asset and local-runtime fingerprints match the currently installed plugin. If a plugin cache was replaced beneath a still-running process, or a healthy process serves older UI or Python behavior, `start` replaces that exact stale runtime automatically. After a deliberate stop or replacement, it reuses the same private loopback port and token from the user-only connection file, so an existing browser tab and previously supplied link reconnect instead of becoming a dead URL.
...
Keep the resting interface visually quiet: put the compact composite reference in a slim metadata line above the task text inside the main content column, so it consumes a little height instead of reserving a wide side column. The real checkbox stays beside that content. The `OI-n` prefix is the permanent item key; the adjacent P0–P3 control changes only priority and makes the full visible reference read as `OI-n-Px`. P0 is highest, P2 is the neutral default, and P3 is lowest. The task text is the edit control. Clicking or pressing it creates one inline textarea at the same location; `Enter` saves, `Shift+Enter` inserts a line break, `Escape` cancels, and leaving the editor saves. An unchanged edit sends no mutation. Never pre-render a text input or separate Edit button beneath every item.
...
- The server snapshots its generic HTML, CSS, and JavaScript at startup and exposes SHA-256 fingerprints for that complete browser shell and the running Python implementation. Removing or replacing the plugin cache cannot strand a running API with a 404, outdated browser shell, or stale server behavior; `start` refuses to reuse a runtime when either fingerprint differs from the installed plugin.
...
- Double-click a non-control part of a row to pin that same detail popover open, or focus the task text and press `Alt+Enter`. Once pinned, treat the explanation as an interactive non-modal dialog: show a close button in its top-right corner, let that button or `Escape` dismiss it, and return focus to the disclosure after the close button is used without immediately reopening its focus preview. `Escape` must cancel any delayed single-click edit from the disclosure gesture and never turn the task text into an editor. After pointer-opened details, clear the double-click selection and blur the task title so dismissal simply returns the row to rest; after keyboard-opened details, preserve task-title focus so keyboard navigation remains usable. The dialog owns its pointer events, so clicks never reach the row underneath; do not prevent the browser's normal text selection, and never interpret a double-click inside the dialog as a row toggle. A single task-text click still enters editing, checkbox clicks still control completion, and drag/reorder controls never trigger row disclosure.
- Keep an active row's compact **Open in Codex** link visibly available without requiring row hover. Keep secondary drag and keyboard-move controls quiet until hover, focus, or a no-hover pointer environment. Use the installed Codex app's `codex://threads/new?prompt=...` deep link to open a fresh normal task with a prefilled, unsent prompt. Include the composite reference, title, status, optional explanation, capture reason, and bounded saved details; never include the tokenized ledger URL or runtime path. End the prompt by asking for a simple explanation and one next action, and explicitly forbid starting the item. The click does not mutate the ledger or claim a transfer. Hide the action for completed and transferred rows.
...
Automated API tests prove validation, migration, atomic priority/edit/toggle/reopen/reorder behavior, stale-revision rejection, token gating, external-file refresh, and the optional `explanation` field. Asset checks prove that the composite reference and P0–P3 control sit in a slim metadata line above the title without a reserved side column or resting title input, that editing creates its textarea on demand, that every row carries a labelled non-modal explanation dialog wired through `aria-describedby` to its dedicated disclosure control and filled with `textContent`, that a pinned dialog owns pointer input without cancelling native text selection, and that the always-visible active-item Codex handoff is prefilled but unsent and excludes runtime secrets. A real-browser pass must still verify the compact top metadata line; priority selection and resulting reorder; stable manual placement; the uncluttered resting state; no detail on row or task-text hover; detail disclosure on caret hover, keyboard focus, and click/tap; pinned close-button and `Escape` dismissal; double-click text selection without activating the row underneath; an item with no `explanation`; click-to-edit behavior; the active-only always-visible **Open in Codex** control and its prefilled prompt; completion snackbar and Undo; pointer drag without accidental detail activation; keyboard movement; transferred read-only presentation; responsive layout; and saved-state feedback before calling a new UI design verified.
