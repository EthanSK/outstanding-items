...
Schema and lifecycle for the task-owned canonical ledger. Load when the footer overflows, the user asks for the full ledger, or a related task needs registering.

The ledger records the user's items. It never authorizes work — see [authority.md](authority.md).

## One source of truth

The agreed task-owned `outstanding-items.json` file is authoritative. The conversation footer is a compact rendering of it, and the Full ledger HTML UI is a live editor for it. Neither is a second ledger.
...
- The user asks for the full list, a plan, a handover, or the Full ledger UI.
...
Do not silently create a ledger before those triggers. When the user has already explicitly asked for the Full ledger UI or durable ledger file, that request supplies the path-creation authority; choose the task-owned output directory when one exists and report it.
...
The server validates IDs, statuses, completion consistency, unique positions, and the owner/authority invariant before every atomic write.
...
2. **Start the UI.** Run the loopback-only server and capture its tokenized URL. The footer's **Full ledger** link points to this HTML URL, never to the raw JSON or legacy Markdown.
...
5. **Reconcile.** On task resume, validate and read the JSON, render the footer from active items, then wait. Restoring a ledger starts nothing. A stale `in-progress` label must be reconciled to `implemented` when material work changed, `planned` when only an agreed route exists, or `requested` when neither is true; none of those labels authorizes resumption.
...
- Bind the editor to `127.0.0.1` only. API calls require the random token in the Full ledger URL, reject foreign Host headers, set no CORS permission, and write no browser storage.
...
- Never put credentials or secret file contents into titles, notes, sections, URLs, examples, or logs.
